...
namespace Ensconce
{
	internal static class Program
	{
		...
		private static void MainLogic(IEnumerable<string> args)
		{
		...
		private static void SetUpAndParseOptions(IEnumerable<string> args)
		{
			var showHelp = false;
			var p = new OptionSet
						{
		...
		public static void StopAndDeleteServicesInDirectory(string directory)
		{
			foreach (var serviceDetails in GetServicesInstalledInDirectory(directory))
			{
				...
				var process = Process.Start("sc", string.Format("delete \"{0}\"", serviceDetails.ServiceName));
				if (process != null)
					process.WaitForExit();
			}
		}
		...
		private static void CompressDir(ZipOutputStream zipStream, DirectoryInfo dir, int dirLength)
		{
			...
			foreach (var file in files)
			{
			    var zipEntry = new ZipEntry(ZipEntry.CleanName(file.FullName.Substring(dirLength)))
			                       {DateTime = file.LastWriteTime, Size = file.Length};
		...
		private static TagDictionary BuildTagDictionary()
		{
			...
			else
			{
				...
				else
				{
					...
					{
					    if (stream != null) configXml = new StreamReader(stream).ReadToEnd();
					}
